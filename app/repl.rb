...
xrepl { puts "The result of 1 + 2 is: #{1 + 2}" }
...
xrepl do
  ...
  puts "This if statement ran because e is nil (a falsey value)." if !e
end
...
xrepl do
  ...
  colors = %w[red blue yellow]
  colors.each_with_index { |color, i| puts "#{color} at index #{i}" }
end
...
xrepl do
  ...
  one_to_ten = (1..10).to_a
  evens = one_to_ten.find_all { |number| number % 2 == 0 }
  puts evens

  puts "======== Create a new array that rejects odd numbers."
  one_to_ten = (1..10).to_a
  also_even = one_to_ten.reject { |number| number % 2 != 0 }
  puts also_even

  puts "======== Create an array that doubles every number."
  one_to_ten = (1..10).to_a
  doubled = one_to_ten.map { |number| number * 2 }
  puts doubled
  ...
  odd_doubled =
    one_to_ten
      .find_all { |number| number % 2 != 0 }
      .map { |odd_number| odd_number * 10 }
  puts odd_doubled
  ...
  uniq_combinations =
    one_to_ten
      .product(one_to_ten)
      .map { |unsorted_number| unsorted_number.sort }
      .uniq
...
xrepl do
  ...
  triples =
    one_to_hundred
      .product(one_to_hundred)
      .map { |width, height| [width, height, Math.sqrt(width**2 + height**2)] }
      .find_all { |_, _, hypotenuse| hypotenuse.to_i == hypotenuse }
      .map { |triangle| triangle.map(&:to_i) }
      .uniq { |triangle| triangle.sort }
      .map do |width, height, hypotenuse|
        [width, height, hypotenuse, (width * height) / 2]
      end
      .sort_by { |_, _, _, area| area }
