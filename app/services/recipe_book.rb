...
class RecipeBook
    ...
    def initialize(potion_defs = $pids[:ingredients], ingredient_defs = $iids)
    ...
    def unlocked?(player, recipe_id)
        player.unlocked_recipes.include?(recipe_id)
    end
    ...
    def can_craft?(player, recipe_id)
        return false unless unlocked?(player, recipe_id)
        required = @potion_defs[recipe_id][:ingredients]
        required.all? do |ing_id, amt|
            player.ingredient_inventory.count { |card| card.id == ing_id } >= amt
        end
    end

    # Consume ingredients and produce a new PotionCard
    def craft(player, recipe_id)
        raise "Recipe not unlocked: #{recipe_id}" unless unlocked?(player, recipe_id)
        unless can_craft?(player, recipe_id)
        ...
        @potion_defs[recipe_id][:ingredients].each do |ing_id, amt|
            amt.times do
                ...
                card = player.ingredient_inventory.deck.draw_pile.find { |c| c.id == ing_id }
                player.ingredient_inventory.discard(card)
            end
        end
        ...
        player.potion_inventory.add(potion_card)
        potion_card
    end
end
