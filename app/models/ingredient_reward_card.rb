    ...
    def calc_position num_cards, index
        ...
        if !@grabbed
            ...
            if num_cards == 2
                ...
                @f_angle = (relative_index / center_index) * max_angle
                @f_pos.y = max_y * ( 1 - ( 1.5 * (normalized_distance)**2 )) + 50 + y_offset

            elsif num_cards > 1
                # Calculate the card's rotation as a fraction of the maximum angle
                @f_angle = (relative_index / center_index) * max_angle
                @f_pos.y = max_y * ( 1 - ( 2 * (normalized_distance)**2 )) + ( 0.75 * (12.5 * num_cards)) + y_offset# +  ( 2 * (normalized_distance)**3 ) ) )# LINEAR: ((1 - normalized_distance) * max_y)
